...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _Box = __webpack_require__(/*! ./components/Box */ 159);
	
	var _Box2 = _interopRequireDefault(_Box);
	
	var _styles = __webpack_require__(/*! ./styles.css */ 174);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _fp = __webpack_require__(/*! lodash/fp */ 160);
	
	var _recompose = __webpack_require__(/*! recompose */ 166);
	...
	var SIZE = 18;
	var SNAP = 3;
	var gc = function gc(rn, xs) {
	  return xs.filter(function (_ref) {
	    var time = _ref.time;
	    return rn - time <= EXPIRY;
	  });
	};
	
	var COLORS = ['red', 'blue', 'yellow', 'black', 'purple', 'orange', 'green'];
	...
	var Box = function (_Component) {
	  ...
	  function Box() {
	    ...
	    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref2 = Box.__proto__ || Object.getPrototypeOf(Box)).call.apply(_ref2, [this].concat(args))), _this), _this.drawPoint = function (ctx, t, point) {
	      ...
	      ctx.restore();
	      ctx.closePath();
	    }, _this.drawCrosshair = function (ctx, point) {
	      ...
	      var yLabel = 'y = ' + y;
	      var ySize = ctx.measureText(yLabel);
	      var xLabel = 'x = ' + x;
	      var xSize = ctx.measureText(xLabel);
	      ctx.beginPath();
	      ctx.moveTo(0, y);
	      ctx.lineTo(width, y);
	      ctx.moveTo(x, 0);
	      ...
	      ctx.closePath();
	      ctx.restore();
	    }, _this.drawFPS = function (ctx, fps, color) {
	      var label = fps.toFixed(0) + ' FPS';
	      var size = ctx.measureText(label);
	      ctx.beginPath();
	      ctx.fillStyle = color;
	      ctx.font = '18px Futura';
	      ctx.fillText(label, 10, 24);
	      ctx.closePath();
	    }, _this.drawRadius = function (ctx, point) {
	      var scalar = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 2;
	      var color = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'black';
	
	      var rotation = 0;
	
	      var _point3 = _slicedToArray(point, 5),
	          x = _point3[0],
	          y = _point3[1],
	          rX = _point3[2],
	          rY = _point3[3],
	      ...
	      var yLabel = rY ? rX.toFixed(2) : '?';
	      var ySize = ctx.measureText(yLabel);
	      var xLabel = rX ? rX.toFixed(2) : '?';
	      var xSize = ctx.measureText(xLabel);
	      ctx.beginPath();
	      ctx.fillStyle = color;
	      ctx.font = '14px Futura';
	    ...
	    }, _this.draw = function (step) {
	      var width = innerWidth;
	      var height = innerHeight;
	      if (_this._ctx) {
	        if (_this._cvs.width !== width || _this._cvs.height !== height) {
	          _this._cvs.width = width;
	          _this._cvs.height = height;
	        }
	        var ctx = _this._ctx;
	        ctx.clearRect(0, 0, width, height);
	
	        var rn = (0, _fp.now)();
	        var trails = gc(rn, (0, _fp.flatten)((0, _fp.values)(_this.props.touches).map(_fp.initial)));
	        var ghosts = [].concat(_toConsumableArray(_this.props.ghosts), _toConsumableArray(trails));
	
	        gc(rn, ghosts).forEach(function (_ref3) {
	          var time = _ref3.time,
	              point = _ref3.point,
	              color = _ref3.color;
	          return _this.drawPoint(ctx, (rn - time) / EXPIRY, point, color);
	        });
	
	        COLORS.forEach(function (color, i) {
	          _this.drawPoint(ctx, 0, [20, 80 + SIZE * 3 * i, SIZE, SIZE], 1 + (color === _this.props.brush ? 0.4 : 0), color);
	        });
	
	        (0, _fp.values)(_this.props.touches).forEach(function (history) {
	          var _last = (0, _fp.last)(history),
	              point = _last.point,
	              color = _last.color;
	
	          if (history) {
	            var start = history[0].point;
	
	            var _start = _slicedToArray(start, 5),
	                startX = _start[0],
	                startY = _start[1],
	                rX = _start[2],
	                rY = _start[3],
	            ...
	            var distance = Math.abs(PERIOD - step);
	            var hypot = Math.hypot(point[0] - startX, point[1] - startY) / 2;
	            var overExtended = hypot / Math.max(rX || SIZE, rY) > SNAP;
	            var radius = overExtended ? SNAP * Math.max(rX || SIZE, rY) : hypot;
	            _this.drawPoint(ctx, 0, start, 1 + WIGGLE * Math.sqrt(distance / (PERIOD / 2)), color);
	            if (overExtended) {
	              var factor = SNAP * Math.max(rX || SIZE, rY) / hypot;
	              _this.drawPoint(ctx, 0, [startX + (point[0] - startX) * factor, startY + (point[1] - startY) * factor, 10, 10], force || 1, color);
	            }
	            _this.drawRadius(ctx, [startX, startY, radius, radius, force], step < 6 ? 6 - step + 1 : 2, color);
	          }
	
	          _this.drawCrosshair(ctx, point, color);
	          _this.drawRadius(ctx, point, 1, color);
	          _this.drawPoint(ctx, 0, point, color);
	        });
	
	        if (_this.props.prevTime !== null) {
	          var delta = (rn - _this.props.prevTime) / 1e3;
	          if (!_this._fps || step === 0) _this._fps = 1 / delta;
	          _this.drawFPS(ctx, _this._fps, _this.props.brush);
	        }
	        _this.props.setPrevTime(rn);
	      }
	      requestAnimationFrame(function () {
	        return _this.draw((step + 1) % PERIOD);
	      });
	    }, _this.action = function (type) {
	      return function (evt) {
	        evt.preventDefault();
	        var _this$props = _this.props,
	            touches = _this$props.touches,
	            ghosts = _this$props.ghosts,
	            setGhosts = _this$props.setGhosts;
	
	        var rn = (0, _fp.now)();
	        var changed = (0, _fp.toArray)(evt.changedTouches);
	        if (evt.type === 'touchstart') {
	          console.log(evt);
	        }
	        var inToolbar = function inToolbar(_ref4) {
	          var pageX = _ref4.pageX,
	              pageY = _ref4.pageY;
	          return pageX < 100 && pageY > 50 && pageY < 450;
	        };
	        if ((0, _fp.any)(inToolbar, changed)) {
	          var _evt = changed.find(inToolbar);
	          var color = COLORS[Math.floor((_evt.pageY - 62) / 54)];
	          _this.props.setBrush(color);
	        }if (type === 'touchstart') {
	          _this.props.setTouches(_extends({}, touches, (0, _fp.fromPairs)(changed.map(function (_ref5) {
	            var pageX = _ref5.pageX,
	                pageY = _ref5.pageY,
	                radiusX = _ref5.radiusX,
	                radiusY = _ref5.radiusY,
	                force = _ref5.force,
	                identifier = _ref5.identifier;
	            return [identifier, [{
	              time: [rn],
	              point: [pageX, pageY, radiusX, radiusY, force],
	              color: _this.props.brush
	            }]];
	          }))));
	        } else if (type === 'touchend' || type === 'touchcancel') {
	          setGhosts([].concat(_toConsumableArray(gc(rn, ghosts)), _toConsumableArray(changed.map(function (_ref6) {
	            var pageX = _ref6.pageX,
	                pageY = _ref6.pageY,
	                radiusX = _ref6.radiusX,
	                radiusY = _ref6.radiusY,
	                force = _ref6.force,
	                identifier = _ref6.identifier;
	            return {
	              time: rn,
	              point: [pageX, pageY, radiusX, radiusY, force],
	              color: (0, _fp.get)('color', (0, _fp.last)(touches[identifier]))
	            };
	          }))));
	          _this.props.setTouches((0, _fp.omit)((0, _fp.map)('identifier', changed), touches));
	        } else {
	          _this.props.setTouches(_extends({}, touches, (0, _fp.fromPairs)(changed.map(function (_ref7) {
	            var pageX = _ref7.pageX,
	                pageY = _ref7.pageY,
	                radiusX = _ref7.radiusX,
	                radiusY = _ref7.radiusY,
	                force = _ref7.force,
	                identifier = _ref7.identifier;
	            return [identifier, [].concat(_toConsumableArray(touches[identifier]), [{
	              time: rn,
	              point: [pageX, pageY, radiusX, radiusY, force],
	              color: (0, _fp.get)('color', (0, _fp.last)(touches[identifier])) || _this.props.brush
	            }])];
	          }))));
	        }
	      };
	    }, _this.getContext = function (cvs) {
	      if (!cvs) return;
	      ['touchstart', 'touchend', 'touchmove', 'touchcancel'].forEach(function (name) {
	        cvs.addEventListener(name, _this.action(name), false);
	      });
	      _this._cvs = cvs;
	      _this._ctx = cvs.getContext('2d');
	...
	exports.default = (0, _fp.compose)((0, _recompose.withState)('brush', 'setBrush', 'black'), (0, _recompose.withState)('prevTime', 'setPrevTime', null), (0, _recompose.withState)('touches', 'setTouches', {}), (0, _recompose.withState)('ghosts', 'setGhosts', []))(Box);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _ = __webpack_require__(/*! ./lodash.min */ 161).runInContext();
	module.exports = __webpack_require__(/*! ./fp/_baseConvert */ 163)(_, _);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */}.call(exports, (function() { return this; }()), __webpack_require__(/*! ./../webpack/buildin/module.js */ 162)(module)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var mapping = __webpack_require__(/*! ./_mapping */ 164),
	    fallbackHolder = __webpack_require__(/*! ./placeholder */ 165);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var shallowEqual = _interopDefault(__webpack_require__(/*! fbjs/lib/shallowEqual */ 167));
	var hoistNonReactStatics = _interopDefault(__webpack_require__(/*! hoist-non-react-statics */ 168));
	var changeEmitter = __webpack_require__(/*! change-emitter */ 169);
	var $$observable = _interopDefault(__webpack_require__(/*! symbol-observable */ 170));
...
/* 167 */
/*!************************************************!*\
  !*** ./~/recompose/~/fbjs/lib/shallowEqual.js ***!
  \************************************************/
/***/ (function(module, exports) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _ponyfill = __webpack_require__(/*! ./ponyfill */ 172);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	// load the styles
	var content = __webpack_require__(/*! !../~/css-loader!./styles.css */ 175);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports = module.exports = __webpack_require__(/*! ../~/css-loader/lib/css-base.js */ 176)(undefined);
	// imports
...
/***/ (function(module, exports, __webpack_require__) {
	...
	*/
	var stylesInDom = {},
		...
		fixUrls = __webpack_require__(/*! ./fixUrls */ 178);
